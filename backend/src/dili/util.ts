...
const airstackAPIKeys = [
  //TODO move to config  and gitignore
...
export const getWeb3BioNextId = async (pk: string) => {
  try {
    const res = await rest_api_save_to_db(
      "https://api.web3.bio/profile/" + pk,
      "get",
    );
    return res;
  } catch (error) {
    console.error(
      "error in getWeb3BioNextId()... if its a 404 ignore it b/c that just means they had no data: " +
        error,
    );
    return [];
  }
...
export const airStackFarCaster = async () => {
  ...
  for (let i = 0; i < pagelimit; i++) {
    const curQuery = `query GetAllFarcasterUsernamesAndEthAddresses {
      Socials(
    ...
    console.log(
      "data?.Socials?.pageInfo.nextCursor " +
        data?.Socials?.pageInfo.nextCursor,
    );
    console.log(
      "data?.Socials?.pageInfo.hasNextPage " +
        data?.Socials?.pageInfo.hasNextPage,
    );
    ...
    for (let x = 0; x < lastDownload.length; x++) {
      ...
      const resumejson = {
        pubkey: row.userAssociatedAddresses[0],
        preferredname: row.profileDisplayName,
        preferredtitle: row.fnames[0] || "",
        description: row.profileBio,
        preferredlocation: null,
        address: row.userAssociatedAddresses[0],
      ...
      console.log(
        "----------- Finished inserting into supabase -- \n " +
...
export const crawlEAS = async () => {
  ...
  const payload_all_eass = {
    query: eas_get_distinct_recipeints,
    variables: {
      distinct: "recipient",
    },
  };

  const alleasrecp = await rest_api_save_to_db(
  ...
  for (let i = 0; i < wallets.length; i += batchSize) {
    ...
    if (batch.length > 0) {
      try {
        ...
        const { data, error } = await fetchQuery(XMTPOnlyQuery, {
          address: batch,
        });
        userData = data;
        ...
        if (userData?.XMTPs?.XMTP) {
          ...
          for (let s = 0; s < userData.XMTPs.XMTP.length; s += 1) {
            ...
            const { error3 } = await supabase
              .from("people_search")
              .update({ on_xmtp: e.isXMTPEnabled })
              .eq("pk", e.owner.addresses[0]);
          }
        }
      } catch (err) {
      ...
      if (userData?.XMTPs.XMTP && userData?.XMTPs.XMTP) {
        //@ts-ignore
        airstackfound =
          userData?.XMTPs?.XMTP?.map((e) => e.owner.addresses[0]) || [];
      }
      console.log("airstackfound=" + airstackfound);
      //@ts-ignore

      //BOOKMARK
      const notfromairstack = batch.filter((w) => !airstackfound.includes(w));
      console.log("notfromairstack=" + JSON.stringify(notfromairstack));

      const xanswers = await x_client.canMessage(notfromairstack);
