...
const GITCOIN_SUBGRAPH_ENDPOINT =
  "https://api.thegraph.com/subgraphs/name/wslyvh/gitcoin-passport-score";

export async function fetchGitcoinPassportUsers() {
  const batchSize = 1000;
  let lastUserId = 21000;
  const users = [];

  while (lastUserId) {
    const graphqlQuery = `
        {
          tokens(first: ${batchSize},orderBy: index, orderDirection:desc, where: { index_lt: ${lastUserId} }) {
            id
            index
            tokenId
            owner
