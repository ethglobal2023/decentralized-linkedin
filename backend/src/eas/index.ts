...
import {Offchain, OFFCHAIN_ATTESTATION_VERSION, PartialTypedDataConfig,} from "@ethereum-attestation-service/eas-sdk";
import {hexlify, Signature, verifyTypedData} from "ethers";
import {CreateEasRequest} from "./create-attestation.js";
...
export const signatureIsFromAttester = (attester: string, request: VerifyEasRequest) => {
...
export const extractAddressFromAttestation = (attestation: VerifyEasRequest) => {
    const sig = Signature.from({v: attestation.sig.signature.v, r: hexlify(attestation.sig.signature.r), s: hexlify(attestation.sig.signature.s)}).serialized;
    return verifyTypedData(attestation.sig.domain, attestation.sig.types, attestation.sig.message, sig);
}
