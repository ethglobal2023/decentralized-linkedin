...
const confirmSchema = Joi.object<
  MessageWithSignature<ConfirmManualVerificationMessage>
>({
  message: {
...
export const confirmVerification = async (
  ...
  const { error: validationError, value } = confirmSchema.validate(req.body, {
    allowUnknown: true,
  });
  if (validationError) {
  ...
  if (error) {
    ...
    return res.status(500).json({
      error: `Failed to mark verification request as fulfilled: ${error}`,
    });
  }
